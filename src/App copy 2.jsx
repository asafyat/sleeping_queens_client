...
const callGemini = async (prompt) => {
  ...
  try {
    ...
    let text = data.candidates?.[0]?.content?.parts?.[0]?.text || "הכוכבים מעוננים היום...";
    
    // --- ניקוי תווים מיוחדים (Markdown) ---
    // מסיר כוכביות (*), סולמיות (#) וגרשיים (`) כדי שהטקסט יראה נקי
    text = text.replace(/[\*\#\`]/g, '');
...
class MockGameEngine {
  ...
  cpuAutoPlay() {
    ...
    if (!cpu || !cpu.name.includes("מחשב")) return; 
    ...
    if (discard) this.finishTurn(cpuId, [discard], `המחשב זרק ${discard.type === 'number' ? 'מספר' : 'קלף'}`, false);
  }
...
/* --- KID MODE THEME --- */
.theme-kid { 
  background: linear-gradient(180deg, #87CEEB 0%, #E0F7FA 100%); /* Sky Blue */
  color: #000; 
  font-family: 'Comic Sans MS', 'Chalkboard SE', sans-serif;
}

.theme-kid .lobby-card {
  border: 5px solid #FF69B4; /* Hot Pink Border */
  border-radius: 30px;
  box-shadow: 0 10px 0px rgba(0,0,0,0.2);
}

.theme-kid .btn-primary {
...
.theme-kid .btn-primary:active { transform: scale(0.95); box-shadow: 0 2px 0 #d35400; }
...
.theme-kid .card:hover { transform: scale(1.1) rotate(2deg); }

.theme-kid .message-bar {
  background: #FFFACD;
...
.theme-default { background: linear-gradient(135deg, #2c3e50 0%, #4a148c 100%); color: #333; }
.theme-default .card { border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
.theme-default .btn { border-radius: 8px; }

.toggle-kid-mode { position: absolute; top: 10px; left: 10px; background: white; border: 2px solid #333; border-radius: 20px; padding: 5px 10px; cursor: pointer; font-size: 0.9rem; z-index: 100; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
`;
...
const getCardVisual = (card) => {
  ...
  switch (card.type) {
    case 'king': return { emoji: '🤴', color: '#FFF8E1', label: 'מלך', icon: Crown };
    case 'knight': return { emoji: '⚔️', color: '#ECEFF1', label: 'אביר', icon: Sword };
    case 'potion': return { emoji: '🧪', color: '#E8F5E9', label: 'שיקוי', icon: FlaskConical };
    case 'dragon': return { emoji: '🐉', color: '#FFEBEE', label: 'דרקון', icon: Shield };
    case 'wand': return { emoji: '🪄', color: '#F3E5F5', label: 'שרביט', icon: Wand2 };
    case 'jester': return { emoji: '🃏', color: '#FFF3E0', label: 'ליצן', icon: Sparkles };
    case 'number': return { emoji: card.value, color: '#E3F2FD', label: 'מספר', icon: null };
    case 'queen':
       let emoji = '👸';
       if (card.name.includes('Rose') || card.name.includes('וורדים')) emoji = '🌹';
       else if (card.name.includes('Dog') || card.name.includes('כלבים')) emoji = '🐶';
       else if (card.name.includes('Cat') || card.name.includes('חתולים')) emoji = '🐱';
       else if (card.name.includes('Sun') || card.name.includes('חמניות')) emoji = '🌻';
       else if (card.name.includes('Moon') || card.name.includes('ירח')) emoji = '🌙';
       else if (card.name.includes('Heart') || card.name.includes('לבבות')) emoji = '❤️';
       else if (card.name.includes('Star') || card.name.includes('כוכבים')) emoji = '⭐';
       return { emoji, color: '#FCE4EC', label: card.name.replace(' Queen', '').replace('מלכת ה', ''), icon: Crown };
...
export default function App() {
  ...
  const [isKidMode, setIsKidMode] = useState(false); // 🎈 Kid Mode Toggle
  ...
  const askAdvisor = async () => {
    ...
    if (!player) {
      setAiContent("לא ניתן למצוא את נתוני השחקן כרגע.");
      setAiLoading(false);
      return;
    }
  ...
  if (view === 'lobby') {
    return (
      <div className={`app-container ${isKidMode ? 'theme-kid' : 'theme-default'}`}>
        <style>{styles}</style>
        
        {/* Toggle Button */}
        <button className="toggle-kid-mode" onClick={() => setIsKidMode(!isKidMode)}>
          {isKidMode ? "חזור למצב רגיל 👔" : "מצב ילדים 🎈"}
        </button>

        <div className="lobby-card">
          <h1 className="lobby-title">{isKidMode ? "🏰 הממלכה הקסומה" : "👑 מלכות ישנות"}</h1>
          <p className="lobby-subtitle">{isKidMode ? "בואו להעיר את המלכות!" : "משחק קלפים אסטרטגי"}</p>
          ...
          <div className="input-group">
            <input 
              ...
              placeholder={isKidMode ? "איך קוראים לך?" : "הכנס שם"}
              value={playerName} 
          ...
          <button className="action-btn btn-create" onClick={createGame}>
            {isKidMode ? "התחל משחק חדש! 🚀" : "צור משחק חדש"}
          </button>

          {roomId && !playerId && (
            <div className="room-id-display">
               <strong>{isKidMode ? "מספר החדר" : "Room ID"}:</strong> {roomId}
               <br/>
               <small>{isKidMode ? "(שתף עם חברים)" : "(Share this ID with friends)"}</small>
            </div>
          )}

          <div className="divider">{isKidMode ? "או הצטרף לחדר" : "OR Join Existing"}</div>

          <div className="input-group">
            <input 
              className="styled-input"
              placeholder={isKidMode ? "הדבק מספר חדר..." : "Paste Room ID here..."}
              value={roomId} 
          ...
          <button className="action-btn btn-join" onClick={joinGame}>
            {isKidMode ? "הצטרף לחדר 🚪" : "Join Room 🚀"}
          </button>
    ...
    return (
      <div className={`app-container ${isKidMode ? 'theme-kid' : 'theme-default'}`}>
        ...
        <div className="lobby-card">
            ...
            <h2 className="lobby-title" style={{marginTop: '10px'}}>{isKidMode ? "כל הכבוד!" : "GAME OVER"}</h2>
            <h3>{isKidMode ? `המנצח הוא: ${winnerName}` : `Winner: ${winnerName}`}</h3>
            <button className="action-btn btn-create" onClick={() => window.location.reload()}>
                {isKidMode ? "שחק שוב" : "Play Again"}
            </button>
        </div>
      </div>
    );
    ...
    <div className={`app-container ${isKidMode ? 'theme-kid' : 'theme-default'}`}>
      ...
      <div className="game-board">
          ...
          <div className="top-bar">
             <div>{isKidMode ? "חדר" : "Room"}: <strong>{gameState.id}</strong></div>
             <div>{gameState.started ? (isMyTurn ? (isKidMode ? "🟢 תורך!" : "🟢 Your Turn") : (isKidMode ? "🔴 מחכים..." : "🔴 Waiting...")) : (isKidMode ? "ממתין..." : "Not Started")}</div>
             {!gameState.started && <button className="start-btn" onClick={startGame}>{isKidMode ? "התחל" : "START GAME"}</button>}
          </div>

          {/* System Messages */}
          {gameState.lastMessage && (
            <div className="message-bar">
              <span><strong>{isKidMode ? "מה קרה:" : "Last Action:"}</strong> {gameState.lastMessage}</span>
              <button className="btn-bard" onClick={askBard} title="שיר!">
                 <Music size={12} /> {isKidMode ? "שיר!" : "Sing!"}
              </button>
            </div>
          )}
            ...
            {gameState.players.filter(p => p.id !== playerId).map(p => (
              <div key={p.id} className="opponent-card">
                ...
                <div style={{fontSize: '0.9rem', color: '#666'}}>
                    {isKidMode ? "קלפים" : "Cards"}: <strong>{p.hand.length}</strong> | {isKidMode ? "נקודות" : "Score"}: <strong>{p.score}</strong>
                </div>
                ...
                <div className="card-row" style={{transform: 'scale(0.85)', marginTop: '5px'}}> 
                  ...
                  {p.queensAwake.length === 0 && <span style={{fontSize: '12px', color: '#999', marginTop: '10px'}}>{isKidMode ? "אין מלכות" : "No Queens"}</span>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="table-center">
            <div className="sleeping-queens">
              <h3>
                {isKidMode ? "מלכות ישנות" : "Sleeping Queens"} ({gameState.queensSleeping.length})
                {selectedType === 'king' && <span style={{color: '#d32f2f', fontSize: '0.8em', marginLeft: '10px'}}> {isKidMode ? "(בחר מלכה!)" : "(Select a Queen!)"}</span>}
                
                {gameState.pendingRoseWake && isMyTurn && (
                    <div style={{color: '#E91E63', fontWeight: 'bold', animation: 'pulse 1s infinite', marginTop: '5px'}}>
                       {isKidMode ? "🌹 בונוס ורד! בחר עוד אחת! 🌹" : "🌹 ROSE BONUS! Pick another queen! 🌹"}
                    </div>
                )}
              </h3>
            ...
            <div className="discard-pile">
                <h3>{isKidMode ? "ערימת זריקה" : "Discard Pile"}</h3>
                {gameState.discardPile && gameState.discardPile.length > 0 ? (
                    ...
                    <div className="empty-slot">{isKidMode ? "ריק" : "Empty"}</div>
                )}
            </div>
          </div>
          ...
          <div className={`my-area ${isMyTurn ? 'active-turn' : ''}`}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                 <div>
                    <h3>{myPlayer?.name} ({isKidMode ? "אני" : "You"}) - {isKidMode ? "נקודות" : "Score"}: {myPlayer?.score}</h3>
                 </div>
                 
                 {/* AI BUTTON */}
                 {gameState.started && (
                    <button className="btn-advisor" onClick={askAdvisor} disabled={!isMyTurn}>
                      <ScrollText size={16} /> ✨ {isKidMode ? "שאל את הפיה" : "Ask Advisor"}
                    </button>
                 )}
              </div>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                <div>
                    <h4 style={{margin: '5px 0'}}>{isKidMode ? "המלכות שלי:" : "My Awake Queens:"}</h4>
                    ...
                    }) : <span style={{color: '#999', alignSelf: 'center'}}>{isKidMode ? "אין לך מלכות עדיין" : "You have no queens yet"}</span>}
                    </div>
                </div>

                <div>
                    <h4 style={{margin: '5px 0'}}>{isKidMode ? "הקלפים שלי:" : "My Hand:"}</h4>
                    <div className="card-row">
            ...
            <div style={{marginTop: '20px', height: '50px'}}>
              {selectedCardIds.length > 0 && (selectedType === 'number' || selectedType === 'jester') && (
                <button onClick={() => playMove(null)} className="start-btn" style={{backgroundColor: '#2196F3', width: '250px'}}>
                  {isKidMode ? (selectedType === 'jester' ? 'הפעל ליצן 🃏' : 'זרוק מספרים') : (selectedType === 'jester' ? 'Play Jester 🃏' : 'Play Selected Numbers')}
                </button>
              )}
            </div>
          ...
          <div className="modal-overlay" onClick={() => setAiModalOpen(false)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              ...
              <h2 style={{color: '#4a148c', display:'flex', alignItems:'center', justifyContent:'center', gap:10}}>
                  {aiType === 'advisor' && <><Sparkles size={24} /> {isKidMode ? "הפיה הקסומה" : "היועץ המלכותי"}</>}
                  {aiType === 'lore' && <><BookOpen size={24} /> {isKidMode ? "אגדות מלכותיות" : "Royal Legends"}</>}
                  {aiType === 'bard' && <><Music size={24} /> {isKidMode ? "הפייטן המלכותי" : "The Royal Bard"}</>}
                  {aiType === 'spy' && <><Eye size={24} /> {isKidMode ? "השדון הסקרן" : "Spy Report"}</>}
              </h2>

              {aiLoading ? (
                  <div style={{padding:20}}><Loader2 className="animate-spin" size={32} style={{margin:'0 auto'}}/> {isKidMode ? "מתייעץ עם הקסם..." : "Consulting the magic..."}</div>
              ) : (
